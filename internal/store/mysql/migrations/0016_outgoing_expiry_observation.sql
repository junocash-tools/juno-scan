ALTER TABLE outgoing_outputs
  ADD COLUMN expired_height BIGINT NULL;
...
SET expired_height = (
  ...
  WHERE e.kind = 'OutgoingOutputExpired'
    ...
    AND JSON_UNQUOTE(JSON_EXTRACT(e.payload, '$.txid')) = o.txid
    AND JSON_UNQUOTE(JSON_EXTRACT(e.payload, '$.action_index')) = CAST(o.action_index AS CHAR)
)
...
CREATE INDEX outgoing_outputs_expired_height_idx ON outgoing_outputs(expired_height);
