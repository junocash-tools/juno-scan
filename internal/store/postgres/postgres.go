...
func (s *Store) ListWalletNotesPage(ctx context.Context, walletID string, query store.NotesQuery) ([]store.Note, *store.NotesCursor, error) {
	...
	if query.MinValueZat > 0 {
		sb.WriteString(fmt.Sprintf(" AND value_zat >= $%d", argN))
	...
	sb.WriteString(fmt.Sprintf(" ORDER BY height, txid, action_index LIMIT $%d", argN))
	args = append(args, query.Limit+1)

	rows, err := s.pool.Query(ctx, sb.String(), args...)
	if err != nil {
		return nil, nil, fmt.Errorf("postgres: list notes: %w", err)
	}
	defer rows.Close()
...
func (s *Store) WalletNoteStatuses(ctx context.Context, walletID string, refs []store.NoteRef) (store.WalletNoteStatusSnapshot, error) {
	...
	if err != nil {
		return out, fmt.Errorf("postgres: note statuses begin: %w", err)
	}
	defer func() { _ = tx.Rollback(ctx) }()

	if err := tx.QueryRow(ctx, `SELECT value FROM scanner_metadata WHERE key='event_epoch'`).Scan(&out.EventEpoch); err != nil {
		return out, fmt.Errorf("postgres: note statuses event epoch: %w", err)
	}
	if err := tx.QueryRow(ctx, `SELECT height, hash FROM blocks ORDER BY height DESC LIMIT 1`).Scan(&out.AsOfScannerHeight, &out.AsOfScannerHash); err == nil {
		...
		return out, fmt.Errorf("postgres: note statuses tip: %w", err)
	}
	if err := tx.QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM wallets WHERE wallet_id=$1 AND disabled_at IS NULL)`, walletID).Scan(&out.WalletFound); err != nil {
		return out, fmt.Errorf("postgres: note statuses wallet: %w", err)
	}
	...
	if out.WalletFound && out.TipFound {
		...
		query.WriteString(`
			...
			FROM notes
			WHERE wallet_id=$1 AND (txid, action_index) IN (`)
		args := []any{walletID}
		argN := 2
		included := 0
		for _, ref := range refs {
			...
			query.WriteString(fmt.Sprintf("($%d,$%d)", argN, argN+1))
		...
		if included > 0 {
			...
			if err != nil {
				return store.WalletNoteStatusSnapshot{}, fmt.Errorf("postgres: note statuses query: %w", err)
			}
			for rows.Next() {
				var n store.Note
				var divIdx int64
				...
				n.WalletID = walletID
				if err := rows.Scan(
					&n.TxID, &n.ActionIndex, &n.Height, &n.Position, &divIdx, &n.RecipientAddress, &n.ValueZat,
					&memo, &n.NoteNullifier, &n.PendingSpentTxID, &n.PendingSpentAt, &pendingExpiryHeight,
					&n.SpentHeight, &n.SpentTxID, &confirmedHeight, &spentConfirmedHeight, &n.CreatedAt,
				); err != nil {
					rows.Close()
					return store.WalletNoteStatusSnapshot{}, fmt.Errorf("postgres: note statuses scan: %w", err)
				}
				if n.ActionIndex < 0 || divIdx < 0 || divIdx > int64(^uint32(0)) {
					rows.Close()
					return store.WalletNoteStatusSnapshot{}, store.ErrInvalidWalletNoteState
				}
				n.DiversifierIndex = uint32(divIdx)
			...
			if err := rows.Err(); err != nil {
				rows.Close()
				return store.WalletNoteStatusSnapshot{}, fmt.Errorf("postgres: note statuses rows: %w", err)
			}
			rows.Close()
		}
	}
	if err := tx.Commit(ctx); err != nil {
		return store.WalletNoteStatusSnapshot{}, fmt.Errorf("postgres: note statuses commit: %w", err)
	}
	return out, nil
}
...
func (s *Store) ListWalletOutgoingOutputsPage(ctx context.Context, walletID string, query store.OutgoingOutputsQuery) ([]store.OutgoingOutput, *store.NotesCursor, error) {
	...
	FROM outgoing_outputs
	WHERE wallet_id = $1 AND mined_height IS NOT NULL
	`)
	args := []any{walletID}
	argN := 2
	...
	if query.Cursor != nil {
		...
		sb.WriteString(fmt.Sprintf(" AND (mined_height, txid, action_index) %s ($%d, $%d, $%d)", op, argN, argN+1, argN+2))
		args = append(args, query.Cursor.Height, query.Cursor.TxID, query.Cursor.ActionIndex)
		argN += 3
	}
	sb.WriteString(fmt.Sprintf(" ORDER BY mined_height, txid, action_index LIMIT $%d", argN))
	args = append(args, query.Limit+1)

	rows, err := s.pool.Query(ctx, sb.String(), args...)
	if err != nil {
		return nil, nil, fmt.Errorf("postgres: list outgoing outputs page: %w", err)
	}
	defer rows.Close()
	...
	if last.MinedHeight == nil {
		return nil, nil, errors.New("postgres: outgoing output missing mined_height")
	}
	next := &store.NotesCursor{
...
WHERE spent_height IS NULL
  ...
  AND $2::bigint > pending_spent_expiry_height
...
func (t *pgTx) InsertOutgoingOutput(ctx context.Context, o store.OutgoingOutput) (bool, error) {
	...
	var position sql.NullInt64
	if o.Position != nil {
		position = sql.NullInt64{Int64: *o.Position, Valid: true}
	}
	...
	RETURNING 1
	`, o.WalletID, o.TxID, o.ActionIndex, minedHeight, position, mempoolSeenAt, txExpiryHeight, o.RecipientAddress, o.ValueZat, memo, o.OvkScope, recipientScope).Scan(&inserted)
	if err == nil {
		return true, nil
	}
	if !errors.Is(err, pgx.ErrNoRows) {
	...
	SET mined_height = COALESCE(outgoing_outputs.mined_height, $4::bigint),
	    position = COALESCE(outgoing_outputs.position, $5::bigint),
	    mempool_seen_at = COALESCE(outgoing_outputs.mempool_seen_at, $6::timestamptz),
	    tx_expiry_height = COALESCE(outgoing_outputs.tx_expiry_height, $7::bigint)
	WHERE wallet_id = $1 AND txid = $2 AND action_index = $3
	  AND (
	    (outgoing_outputs.mined_height IS NULL AND $4::bigint IS NOT NULL) OR
	    (outgoing_outputs.position IS NULL AND $5::bigint IS NOT NULL) OR
	    (outgoing_outputs.mempool_seen_at IS NULL AND $6::timestamptz IS NOT NULL) OR
	    (outgoing_outputs.tx_expiry_height IS NULL AND $7::bigint IS NOT NULL)
	  )
	`, o.WalletID, o.TxID, o.ActionIndex, minedHeight, position, mempoolSeenAt, txExpiryHeight)
	if err != nil {
...
RETURNING wallet_id, txid, action_index, mined_height, position, confirmed_height, mempool_seen_at, recipient_address, value_zat, memo_hex, ovk_scope, recipient_scope, created_at
